...
export class Queue4Job {
  ...
  static get JOB_NAME() {
    return JOB_NAMES.GLOBAL_SCOPED_JOB_FOR_QUEUE4;
  }
  ...
  static get QUEUE_NAME() {
    return QUEUE_NAMES.QUEUE4;
  }

  static handleJob(job: Job) {
    const logger = new Logger(Queue4Job.name);
    logger.log(`${JOB_NAMES.GLOBAL_SCOPED_JOB_FOR_QUEUE4}: Started`);
    const { data } = job;
    ...
    logger.log(`${JOB_NAMES.GLOBAL_SCOPED_JOB_FOR_QUEUE4}: Completed`);
  }
}

export default Queue4Job.handleJob.bind(Queue4Job);
