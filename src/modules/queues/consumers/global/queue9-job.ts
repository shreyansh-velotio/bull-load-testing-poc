...
export class Queue9Job {
  ...
  static get JOB_NAME() {
    return JOB_NAMES.GLOBAL_SCOPED_JOB_FOR_QUEUE9;
  }
  ...
  static get QUEUE_NAME() {
    return QUEUE_NAMES.QUEUE9;
  }

  static handleJob(job: Job) {
    const logger = new Logger(Queue9Job.name);
    logger.log(`${JOB_NAMES.GLOBAL_SCOPED_JOB_FOR_QUEUE9}: Started`);
    const { data } = job;
    ...
    logger.log(`${JOB_NAMES.GLOBAL_SCOPED_JOB_FOR_QUEUE9}: Completed`);
  }
}

export default Queue9Job.handleJob.bind(Queue9Job);
